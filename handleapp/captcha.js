...
var _handler = {
    ...
    validata:function(header,response){
            response.writeHead(200,{
                "Content-Type":"text/plain"
            });
            if(header.session.session.vcode&&header.session.session.vcode==header.queryString.vcode){
                response.end("1");
            }else{
                response.end("0");
            }
            delete header.session.session.vcode;
    }/*,
    piccaptcha:function(header,response){
        var picid = header.queryString.picid?header.queryString.picid:"",
    ...
    validata2:function(header,response){
        var getValidataCode = edge.func(function () {/*
         ...
         try
         {
         using (System.Drawing.Image bitmap = new Bitmap(100, 20))
         {
         ...
         string dataStr = "0123456789abcdefghijklmnopqrstuvwyz";
         int length = dataStr.Length;

         for (int i = 0; i < 4; i++)
         {
         vcode += dataStr.ToCharArray()[random.Next(0, length)];
         }
         vcode = vcode.ToUpper();
         g.Clear(Color.White);
         g.DrawString(vcode, new Font("宋体", 16), Brushes.Black, new PointF(1, 1));
         for (int i = 0; i < 15; i++)
         {
         if (i / 2 == 0)
         {
         g.DrawLine(Pens.Black, new Point(r.Next(0, bitmap.Size.Width), r.Next(0, bitmap.Size.Height)), new Point(r.Next(0, bitmap.Size.Width), r.Next(0, bitmap.Size.Height)));
         }
         }
         }
